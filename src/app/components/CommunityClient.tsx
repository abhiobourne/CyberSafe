// src/components/CommunityClient.tsx
"use client";
      ...
      <div className="flex flex-wrap gap-2 mt-4">
        {selectedFilters.map((filter) => (
          <div key={filter} className="flex items-center bg-green-500 text-white px-3 py-2 rounded-full">
            {filter}
            <button className="ml-2 text-white hover:text-red-500" onClick={() => toggleFilter(filter)}>
              <FaTimes />
            </button>
          </div>
        ))}
        ...
        {filteredExperiences.map((exp) => (
          <div key={exp.id} className="p-4 bg-[#1E293B] rounded-lg shadow-md">
            ...
            {/* eslint-disable-next-line @typescript-eslint/no-explicit-any*/}
                Posted On - {new Date((exp.createdAt as any).seconds * 1000).toLocaleString()}
                </p>
          </div>
        ))}
      </div>
